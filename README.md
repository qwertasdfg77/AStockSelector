...
> 上方为 App 真实运行截图；股票结果只代表截图时的本地缓存筛选结果，不构成投资建议。

## 项目特点

- 纯 Android 端运行，Kotlin + Jetpack Compose。
- 支持手机端联网更新 A 股列表和日 K 数据。
- 新浪数据源优先；请求失败、空数据或最新日期未达到目标收盘日时，自动改用腾讯备用源。
- 本地 SQLite 缓存，默认保留约 270 个交易日。
- 智能更新：先判断缓存是否为收盘最新数据，再只补缺目标交易日、失败或新增的股票。
- 股票主表保留完整有效股票；降低最低成交额时，只补新进入门槛范围且缺少最新 K 线的股票。
...
关注上升趋势中的连续 3 至 5 根阳线，要求实体占振幅至少 20%、出现非涨停光头阳，并至少有一天同时达到前一日 3 倍量和前 5 日均量 1.5 倍。

### 拉升三阳

关注上升趋势中最近 3 根实体至少占振幅 20% 的阳线，要求收盘价和成交量逐日提高，第三根阳线收盘创近 5 日新高并直接形成信号。
...
```text
点击“智能更新并筛选”
-> 先按中国时间估算目标收盘交易日上限，再用样本 K 线确认数据源实际最新交易日
-> 检查本地缓存 latest date
-> 缓存最新、规则未变化且无可重试缺口：复用上次全量筛选结果
-> 缓存过期：只更新缺目标交易日的股票
-> 缓存最新但有失败：只补失败股票
-> 最低成交额下调：只补新进入范围且缺少最新 K 线的股票
...
- `android-ci.yml`：每次推送和 PR 自动检查版本元数据、运行单元测试与 Android Lint，并构建 debug APK。
